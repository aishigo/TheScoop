...
function createComment(url, request) {
	const response = {};
	const c = request.body && request.body.comment;

	if (c && c.body && c.username && c.articleId && database.users[c.username] &&
		database.articles[c.articleId]) {
		const comment = {
...
function deleteComment(url, request) {
	...
	if (savedComment) {
		...
		userCommentIds.splice(userCommentIds.indexOf(id), 1);
		const userArticleCommentIds = database.articles[savedArticle.id].commentIds;
